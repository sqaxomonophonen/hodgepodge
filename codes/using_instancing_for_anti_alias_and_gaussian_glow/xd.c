...
static uint32_t chunk_add_vertex(struct chunk* chunk, struct vertex v)
{
	assert((chunk->n_vertices+1) < ARRAY_LENGTH(chunk->vertices));
	uint32_t i = chunk->n_vertices++;
	chunk->vertices[i] = v;
	return i;
}

static void chunk_add_triangle(struct chunk* chunk, uint32_t v0, uint32_t v1, uint32_t v2)
{
...
static void chunk_add_circle(struct chunk* chunk, int n, uint32_t x, uint32_t y, uint32_t r0, uint32_t r1, uint32_t color0, uint32_t color1)
{
	...
	for (int i = 0; i < n; i++) {
		...
		uint32_t v = chunk_add_vertex(chunk, (struct vertex) { .a_pos={x0,y0},  .a_color=color0 });
		if (i == 0) v0 = v;
...
static void chunk_add_waveform(struct chunk* chunk)
{
	...
	for (int i = 0; i < n; i++) {
		...
		uint32_t dy = w * 30000.0;
		uint32_t y0 = 32768 - dy;
		uint32_t y1 = 32768 + dy;
		uint32_t v = chunk_add_vertex(chunk, (struct vertex) { .a_pos={x,y0},  .a_color=color });
		if (i == 0) v0 = v;
...
static struct render* new_render(void)
{
	...
	render->program_paint = create_program(
		...
		"void main()\n"
		"{\n"
		"	vec4 p = u_tx * vec4(u_origin + u_scale*(a_pos*(1.0/65536.0)), 0.0, 1.0);\n"
