...
static struct render* new_render(int multisample_log2)
{
	...
	render->program_paint = create_program(
		...
		"	vec2 ip = vec2(mod(i, u_samples.x), float(i / u_samples.x)) / vec2(u_samples);\n"
		"	vec2 u2 = vec2(2.0, 2.0) / u_dim;\n"
		"	vec2 d = u2*ip;\n"
		"\n"
		"	v_color = a_color;\n"
		"	gl_Position = p + vec4(d, 0.0, 0.0);\n"
		"}\n"
	,
		"#version 140\n"
		"in vec4 v_color;\n"
...
int main(int argc, char** argv)
{
	...
	{
		uint16_t v0 = chunk_add_vertex(chunk, (struct vertex) { .a_pos={0,    0    }, .a_color=0xffff00ff });
		uint16_t v1 = chunk_add_vertex(chunk, (struct vertex) { .a_pos={1000, 0    }, .a_color=0xffff00ff });
		uint16_t v2 = chunk_add_vertex(chunk, (struct vertex) { .a_pos={1000, 1000 }, .a_color=0xffff00ff });
		uint16_t v3 = chunk_add_vertex(chunk, (struct vertex) { .a_pos={0,    1000 }, .a_color=0xffff00ff });
