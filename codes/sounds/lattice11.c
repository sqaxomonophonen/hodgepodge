...
#define LATTICE_WIDTH (300)
#define LATTICE_HEIGHT (20)
...
static void lattice_step(struct lattice* l)
{
	...
	for (int y = 1; y < (LATTICE_HEIGHT-1); y++) {
		for (int x = 1; x < (LATTICE_WIDTH-1); x++) {
			...
			float c1 = l->pos[pi+dx] * 0.9;
			float c2 = l->pos[pi-dy] * 0.8;
			float c3 = l->pos[pi+dy];

			float avg = (c0+c1+c2+c3) * 0.25f;

			float f = (avg - l->pos[pi]) * l->speed[pi];
...
int main(int argc, char** argv)
{
	...
	for (int y = 0; y < LATTICE_HEIGHT; y++) {
		for (int x = 0; x < LATTICE_WIDTH; x++) {
			...
			int pi = POS(x,y);
			lattice->damp[pi] = 0.9999f - cos(dd*2) * 0.0001f;
			lattice->bounce[pi] = x < LATTICE_WIDTH/3 ? 0.003f : 0.0f;
			lattice->speed[pi] = x < LATTICE_WIDTH/10 ? 1.0f : 0.7f;
		}
	}
